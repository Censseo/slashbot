---
description: Analyze codebase and specs to update architecture registry, project state context, and local CLAUDE.md files (module and sub-module conventions)
semantic_anchors:
...
| ------ | -------- | ---------------- |
| High-level patterns | `.specforge/memory/architecture-registry.md` | Architectural patterns, technology decisions, interface contracts, anti-patterns |
| Project state context | `specs/CLAUDE.md` | Domain vocabulary, data model state, active contracts, feature dependencies, business invariants, cross-cutting concerns |
| Module conventions | `{module}/CLAUDE.md` | Coding conventions, interface contracts, business invariants, state machines, guard rails, dependency graph, testing conventions |
| Sub-module conventions | `{module}/{subdir}/CLAUDE.md` | Layer-specific patterns, function signatures with spec sources, injected dependencies, expected error types — only generated for directories exceeding complexity threshold |

## Outline

### Phase 1: Discovery

1. **Detect project structure**: Identify main source directories (`src/`, `lib/`, `app/`, `packages/`), detect subdirectories representing distinct modules, look for existing `CLAUDE.md` files.

1. **Load existing registry** (if exists): Read `.specforge/memory/architecture-registry.md`, extract registered patterns, identify what needs updating.
...
1. **Scan specs for in-progress state**:
   ...
   - Load existing `specs/CLAUDE.md` to identify what needs updating
...
1. **Extract project-wide state from docs and specs**:
   ...
   e. **Business Invariants**: Extract business rules and acceptance criteria (Given/When/Then). Categorize as Data invariants (DI-N), Workflow invariants (WI-N), Security invariants (SI-N). Cross-reference with `.specforge/memory/constitution.md` Specification Principles.
...
1. **For each module, evaluate sub-directories for granular context**:
   ...
   a. **Complexity threshold** — generate a sub-module `CLAUDE.md` when ANY of:
...
1. **Update `.specforge/memory/architecture-registry.md`** — high-level content only:

   ```markdown
   # Architecture Registry
   > High-level patterns. Module-specific: `{module}/CLAUDE.md`. Project state: `specs/CLAUDE.md`.
...
1. **Create/Update `specs/CLAUDE.md`**: Use template from `.specforge/templates/specs-context-template.md`. Fill from Phase 2 results. Preserve content between `<!-- MANUAL ADDITIONS START -->` and `<!-- MANUAL ADDITIONS END -->` markers. Skip if specs/ is empty (logged in Phase 2).

1. **Create/Update `{module}/CLAUDE.md`** for each detected module:

   Use template from `.specforge/templates/module-claude-template.md`. Fill sections from Phase 4 steps (a-j): Overview, Structure, Naming, Patterns, Components, Error Handling, Testing, Interface Contracts, Business Invariants, State Machines, Guard Rails, Module Dependency Graph, Gotchas. Preserve MANUAL ADDITIONS markers.
...
1. **Create/Update `{module}/{subdir}/CLAUDE.md`** for qualifying sub-directories (passed Phase 5 threshold). No separate template — content varies by layer type:
...
- `.specforge/memory/architecture-registry.md` — high-level architectural patterns
- `specs/CLAUDE.md` — project state context for spec agents
- `{module}/CLAUDE.md` — module conventions, contracts, invariants
- `{module}/{subdir}/CLAUDE.md` — layer-specific context (qualifying dirs only)
...
| ------- | ------------------------------- | --- |
| `/specforge.specify` | `specs/CLAUDE.md` | Vocabulary consistency — prevents inventing conflicting terms |
| `/specforge.plan` | `architecture-registry.md` + `specs/CLAUDE.md` + `/docs/` directly | Architecture alignment + contract/invariant awareness |
| `/specforge.implement` | `{module}/CLAUDE.md` + `{subdir}/CLAUDE.md` | Conventions + contracts reduce functional drift during coding |
| `/specforge.clarify` | `specs/CLAUDE.md` | Asks questions using correct domain vocabulary |
