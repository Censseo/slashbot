...
2. **Load required context documents**:
   ...
   - `.specforge/memory/architecture-registry.md` — established patterns, technology decisions, component conventions, anti-patterns
...
3. **Check for specialized agents**:
   ...
   - Check if `.claude/agents/specforge/researcher.md` and `.claude/agents/specforge/planner.md` exist
...
1. **Launch researcher agent for codebase analysis**:
   ...
   Use Task tool with:
   - If `.claude/agents/specforge/researcher.md` exists: subagent_type="researcher"
...
2. **For each task group** (sequential groups, all tasks in group planned together):
   ...
   a. **Launch planner agent**:
      ...
      Use Task tool with:
      - If `.claude/agents/specforge/planner.md` exists: subagent_type="planner"
