...
describe('NodeRedManager', () => {
  ...
  describe('constructor', () => {
    it('accepts EventBus instance', () => {
      manager = new NodeRedManager(mockEventBus, '/tmp/test-slashbot');
  ...
  describe('init()', () => {
    ...
    it('adopts stale process if port probe returns 200 (FR-018)', async () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith(
        'nodered:ready',
        expect.objectContaining({ port: 1880 }),
      );
    });
  ...
  describe('start()', () => {
    ...
    it('emits prompt:redraw event on state changes', async () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('prompt:redraw', {});
    });
    ...
    it('returns error if state is disabled', async () => {
      ...
      const newManager = new NodeRedManager(mockEventBus, '/tmp/test-slashbot');
  ...
  describe('stop()', () => {
    ...
    it('emits nodered:stopped event', async () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('nodered:stopped', {});
    });
